            ...
            case WRAPUP:
                ...
                try 
                    ...
                    logger.debug("JobID:" + JobID);
                        ...
                        String filename = getCharacterDataFromElement(line);
                    logger.debug("filename:" + filename);
                        ...
                        if(filename.startsWith("fejl"))
							{
							body += "<br /><br />Følgende trin fejlede:<br />\n";
							body += "Trin " + JobID + " (" + tool + ")";
							}
						else
							{
							String localFileName = ToolsProperties.documentRoot + ToolsProperties.stagingArea + Filename(filename);
							String md5sum = MD5(localFileName);
							logger.debug("md5sum(" + localFileName + "):" + md5sum);

							localFileName = ToolsProperties.documentRoot + ToolsProperties.stagingArea + FilenameRelations(filename);
							String md5sumrel = MD5(localFileName);
							logger.debug("md5sum(" + localFileName + "):" + md5sumrel);
							String href2 = string1 + Filename(filename);
							String href2nometa = string1 + FilenameNoMetadata(filename);
							String href2relations = string1 + FilenameRelations(filename);
							NodeList itemslist = element.getElementsByTagName("item");
							if(itemslist.getLength() > 0)
								{
								if(itemslist.getLength() == 1)
									body += "<br /><br />Følgende trin brugte denne resurse som input:<br />\n";
								else
									body += "<br /><br />Følgende trin brugte disse resurser som input:<br />\n";
								for (int j = 0; j < itemslist.getLength(); ++j) 
									{
									Element item = (Element) itemslist.item(j);
							...
							body += "Trin " + JobID + " (" + tool + ")"
								+  ": <a href=\"" + href2          + "\">inklusiv metadata</a> (md5 checksum: " + md5sum + "), "
								+    "<a href=\"" + href2relations + "\">relationsfil</a> (md5 checksum: " + md5sumrel + "), "
								+    "<a href=\"" + href2nometa    + "\">uden metadata</a><br />\n";
							}
                        }
                    }
                catch (Exception e) 
